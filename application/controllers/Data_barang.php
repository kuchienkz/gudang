        ...
        if ($this->input->post('submit', TRUE) == 'submit') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $simpan = array(
                    ...
                    'harga_beli' => str_replace('.', '', $this->security->xss_clean($this->input->post('harga_beli', TRUE))),
                );
    ...
    public function ajax_barang()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($list as $i) {
                ...
                $row[] = $i->brand;
                $row[] = $i->stok;
                $row[] = '<span class="float-left">Rp.</span><span class="float-right">' . number_format($i->harga_beli, 0, ',', '.') . ',-</span>';
                $row[] = '<span class="float-left">Rp.</span><span class="float-right">' . number_format($i->harga, 0, ',', '.') . ',-</span>';
                $row[] = '<span class="float-left">Rp.</span><span class="float-right">' . number_format($i->harga - $i->harga_beli, 0, ',', '.') . ',-</span>';
